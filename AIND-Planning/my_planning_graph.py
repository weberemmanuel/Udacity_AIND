...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # The set of actions for this level
        a_level = set()
        self.a_levels.append(a_level)

        # For each action check if the preconditions are in the current literals
        # If so add it to the current action set for this level and make sure to
        # Relate the action to its literals.
    ...
    def add_literal_level(self, level):
        ...
        # The set of literal for this level
        s_level = set()
        self.s_levels.append(s_level)

        # For each action in the previous level, add all the litereal that will be
        # implied by executing the action to the set of literal for this level.
        # Make sure to setup the parent child relationship
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        '''
        # Go over the 2 action nodes and check if any effect of one contradict
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        '''
        # Go over the 2 action nodes and check if any effect of one contradict
        # any of the preconditions of each others (both additive and negative)
        for eff in node_a1.action.effect_add:
    ...
    def h_levelsum(self) -> int:
        ...
        :return: int
        '''
        def goal_in_level(s_level,goal):
        ...
        for goal in self.problem.goal:
            ...
            for level in range(len(self.s_levels)):
                if goal_in_level(self.s_levels[level],goal):
                    level_sum += level
                    break
